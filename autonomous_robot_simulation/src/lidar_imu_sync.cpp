/*
@description:
This node synchronizes LiDAR point cloud data with IMU data using message filters.
*/
...
LidarImuSync::LidarImuSync(/* args */) : Node("lidar_imu_sync")
{
    ...
    RCLCPP_INFO(this->get_logger(), "\033[1;34m---->lidar_frame_id: %s \033[0m", lidar_frame_id_.c_str());
...
void LidarImuSync::points_imuCallback(const ImuMsg::ConstSharedPtr &msg1, const PointCloudMsg::ConstSharedPtr &msg2)
{
    ...
    point_cloud_data.header.frame_id = lidar_frame_id_;
    point_cloud_data.header.stamp = msg2->header.stamp;
