...
3. Since no requirements for feature extraction, FAST-LIO2 can support multiple LiDAR inputs including spinning sensors (Velodyne, Ouster) and generic `sensor_msgs/msg/PointCloud2` pipelines, and can be extended to support more.
...
### <span id="1.3">1.3. **Sensor Input Requirements**</span>
This ROS 2 fork consumes standard `sensor_msgs/msg/PointCloud2` and `sensor_msgs/msg/Imu` topics.

Use one of the included configs under `config/`:

- `simulated.yaml`
- `velodyne.yaml`
- `ouster64.yaml`

If your LiDAR already publishes `PointCloud2`, no extra vendor-specific driver package is required by `fast_lio`.
...
### 3.1 Run using ros2 launch
Launch FAST-LIO with the config that matches your sensor pipeline:
```bash
cd <ros2_ws>
. install/setup.bash # use setup.zsh if use zsh
...
You can switch `config_file` to another YAML under `config/`, for example:
...
### 3.2 PointCloud2 and external IMU setup

Edit the selected config file to set the parameters below before running:

1. LiDAR point cloud topic name: ``` lid_topic ```
2. IMU topic name: ``` imu_topic ```
3. Static time offset: ``` time_offset_lidar_to_imu ```
...
In order to validate the robustness and computational efficiency of FAST-LIO in actual mobile robots, we build a small-scale quadrotor which can carry a 70 degree FoV LiDAR and a DJI Manifold 2-C onboard computer with a 1.8 GHz Intel i7-8550U CPU and 8 G RAM, as shown in below.
